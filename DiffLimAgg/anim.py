...
if __name__=="__main__":
    ...
    drift_field = lambda x: -(x-0.5).dot(0.5*np.array([[1,1],[-1,1]])) / (np.sqrt(((x-0.5)**2).sum(axis=1)))[:,None] /10
    #print(force_field(np.array([[1,1.0]])))
    walkers = Walkers2D(N,
                        dt,
                        initial_positions='mixed',
                        position_noise_coefficient=0.001,
